// C:\Dev\NJDRIVE50\android\app\src\main\java\com\njdrive50\app\DriveTrackingService.kt
package com.njdrive50.app

import com.njdrive50.app.db.DriveDao
import com.njdrive50.app.db.DriveDatabase
import com.njdrive50.app.db.DriveFinalizer
import com.njdrive50.app.db.DrivePointEntity
import com.njdrive50.app.db.DriveSessionEntity
...
class DriveTrackingService : Service() {
    ...
    companion object {
        ...
        private const val WAKE_LOCK_TIMEOUT_MS = 10 * 60 * 1000L // 10 minutes, renewed on each location fix
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent == null) {
            ...
            // startForeground() MUST be called within the OS timeout window or the
            // process is killed with ForegroundServiceDidNotStartInTimeException —
            // so promote immediately with a generic notification, then resolve which
            // session (if any) to recover or finalize on the background coroutine.
    ...
    private fun startDriveInternal(driveId: String) {
        ...
        // Session row must exist BEFORE any location fix can be written, since
        // DrivePointEntity is looked up by driveId during finalization. Insert
        // synchronously within this coroutine, then only start GPS updates once
        // that completes — eliminates the race where a fast GPS fix could try to
        // write a point before its parent session row exists.
        scope.launch {
    ...
    private suspend fun recoverOrFinalizeStaleSession() {
        val session = dao.getActiveSession() ?: run {
            // Nothing to recover — the generic notification posted in onStartCommand
            // has no active drive behind it, so shut down cleanly.
            stopForeground(STOP_FOREGROUND_REMOVE)
            stopSelf()
            return
        }
    ...
    // Renew the wake lock on every fix so long drives (>10 min) don't
    // lose CPU wake and silently drop GPS points mid-trip.
    renewWakeLock()
    ...
    // ✅ Add the try/catch here — inside handleLocation
    scope.launch {
        try {
            dao.insertPoint(point)
        } catch (e: Exception) {
            android.util.Log.e("DriveTrackingService", "Failed to insert point", e)
        }
    }
}
    ...
    /**
     * Re-acquires the wake lock with a fresh timeout. Safe to call repeatedly —
     * acquire() on a held PARTIAL_WAKE_LOCK simply extends the timeout window.
     */
