...
export default function DriveSummaryContent({
  ...
  const heroDurationLabel = hasActiveDrive
    ? formatClock(activeDurationSeconds)
    : hasLastDrive
      ? formatClock(Math.round(safeNumber(lastDrive?.totalDurationHours) * 3600))
      : "—"

  const heroDurationTitle = hasActiveDrive
    ? "Active Drive Duration"
  ...
  const lastDriveHoursParts = hasActiveDrive
    ...
    : hasLastDrive
      ? formatHours(safeNumber(lastDrive?.totalDurationHours)).split(" ")
      : ["—", ""]
      ...
      <div className="mx-auto w-full max-w-[42rem] p-4 pt-6 sm:p-6 sm:pt-7">
        ...
        <div className="mt-5 rounded-[28px] border border-[#0A1E5E]/10 bg-[#F7F9FC] p-4 shadow-sm sm:p-5">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <div className="col-span-2 mt-3 rounded-2xl border border-[#0A1E5E]/10 bg-white p-4 shadow-sm sm:col-span-3">
              <p className="text-[11px] leading-tight tracking-[0.16em] text-[#0A1E5E]/55 uppercase">
                Total Miles
