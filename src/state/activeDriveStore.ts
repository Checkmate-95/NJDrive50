...
function normalizeSession(value: unknown): ActiveDriveSession {
  ...
  return {
    ...
    // routeTrail is intentionally NOT restored from persisted storage —
    // see partialize() below for why it's excluded from disk entirely.
  ...
  if (
    ...
    currentSpeed = 0
      ...
      startGlobalTick: () => {
        ...
        // Increment generation so a listener registration promise that
        // resolves after a stop→start→stop cycle can detect it's stale
        // and clean itself up instead of leaking.
        const myGeneration = get()._tickGeneration + 1
        set({ _tickGeneration: myGeneration })
        ...
        CapacitorApp.addListener("appStateChange", ({ isActive }) => {
          ...
          // If the tick loop was stopped/restarted while this promise was
          // pending, or the generation has moved on, this listener is
          // stale — remove it immediately rather than leaving it attached.
          if (
            get()._tickInterval === null ||
            get()._tickGeneration !== myGeneration
          ) {
            void handle.remove()
            return
          }
      ...
      stopGlobalTick: () => {
        // Bump generation immediately so any in-flight addListener promise
        // from a previous startGlobalTick() call knows it's now stale.
        set({ _tickGeneration: get()._tickGeneration + 1 })
    ...
    }),
    {
      ...
      partialize: (state) => ({
        session: {
          ...state.session,
          // Exclude the raw GPS breadcrumb trail from persisted storage.
          // Up to 500 precise lat/lng points sitting in plaintext
